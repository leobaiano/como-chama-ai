...
function vp_get_custom_icons()
{
	// scrape list of icons from fontawesome css
	if( false === ( $icons  = get_transient( 'vp_custom_icons' ) ) )
	{
		$pattern = '/\.(rt-icon-(?:\w+(?:-)?)+):before\s*{\s*content/';
		$subject = file_get_contents(LUXE_BASE_DIR . '/css/scss/plugins/_font-icons.scss');
		...
		set_transient( 'vp_custom_icons', $icons, 60 * 60 * 24 );
	}
